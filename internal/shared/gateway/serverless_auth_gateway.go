...
// Implements customergateway.AuthGateway interface for compatibility
...
// Ensure ServerlessAuthGateway implements AuthGateway interface
var _ customergateway.AuthGateway = (*ServerlessAuthGateway)(nil)
...
// GenerateToken generates token via serverless auth (for future use)
// Maintains same interface as JWTService from tc-golunch-api
func (s *ServerlessAuthGateway) GenerateToken(userID, userType string, additionalClaims map[string]any) (string, error) {
	// For now, return error as token generation should be done directly via Lambda endpoints
	// This maintains interface compatibility with the monolith pattern
	return "", fmt.Errorf("token generation should be done directly via Lambda endpoints (/auth/login, /auth/register, etc)")
}
...
func (s *ServerlessAuthGateway) ValidateToken(tokenString string) (*entity.CustomClaims, error) {
	...
	req.Header.Set("User-Agent", "GoLunch-Core-Service/1.0")
